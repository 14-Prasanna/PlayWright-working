...
Call log:
  ...
  - waiting for locator('p#message.mt-20')
    - locator resolved to <p id="message" class="mt-20"></p>
    - unexpected value ""
...
```ts
  ...
  9  | 
  10 |     const input_field = page.getByPlaceholder("Please enter your Message");
  11 | 
  12 |     await input_field.scrollIntoViewIfNeeded();
  13 | 
  14 |     console.log(await input_field.getAttribute("placeholder"));  
  15 |     await input_field.fill("prasanna");
  16 | 
  17 |     await page.locator("#showInput").click();
  18 | 
  19 |     const input_value = await input_field.inputValue();  
  20 | 
  21 |     await page.waitForTimeout(3000);
  22 | 
  23 |     console.log("input field", input_value);
  24 | 
