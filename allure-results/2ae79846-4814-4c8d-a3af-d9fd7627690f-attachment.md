...
```ts
  ...
  38 |     sum1.fill(a);
  39 | 
  40 |     const sum2 = await page.locator("#sum2");
  41 |     sum2.fill(b);
  42 | 
  43 |     const btn_click = await page.getByRole('button', { name: 'Get Sum' }).click();
  44 |     
  45 |     const display = await page.locator("#addmessage")
  46 | 
> 47 |     console.log(await display.inputValue());
     |                               ^ Error: locator.inputValue: Error: Node is not an <input>, <textarea> or <select> element
  48 | 
  49 |     expect(display).toBe("35");
  50 |     
  51 | })
  52 | 
  53 | test.afterEach(async({page}) => {
  54 |     console.log("Test completed");
  55 | })
```