...
Call log:
  ...
  - waiting for locator('#message')
    3 × locator resolved to <p id="message" class="mt-20"></p>
      - unexpected value ""
...
```ts
  ...
  11 | 
  12 |     const display_msg = page.locator("#message");
  13 | 
  14 |     const input_value = await input_field.inputValue();  
  15 |     const display_value = await display_msg.textContent(); 
  16 | 
  17 |     console.log("Input:", input_value);
  18 |     console.log("Displayed:", display_value);
  19 | 
> 20 |     expect(display_msg).toHaveText("Prasanna");
     |                         ^ Error: expect(locator).toHaveText(expected) failed
  21 | });
  22 | 
```