...
> 32 |   await expect(page.locator('p#message.mt-20')).toHaveText('prasanna');
  ...
  43 |   await expect(sum1).toHaveValue(a);
  44 | 
  45 |   const sum2 = page.locator("#sum2");
  46 |   await sum2.scrollIntoViewIfNeeded();
  47 |   await sum2.clear();
  48 |   await sum2.pressSequentially(b, { delay: 100 });
  49 |   await expect(sum2).toHaveValue(b);
  50 | 
  51 |   await page.getByRole("button", { name: /get sum/i }).click();
  52 | 
  53 |   const display = page.locator("#addmessage");
  54 | 
  55 |   await expect(display).toHaveText("35", { timeout: 10000 });
  56 | 
  57 |   console.log("The value:", await display.textContent());
  58 | });
  59 | 
