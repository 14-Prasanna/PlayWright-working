...
Call log:
  ...
  - waiting for locator('#addmessage')
    32 × locator resolved to <p class="mt-20" id="addmessage"></p>
       - unexpected value ""
...
```ts
  ...
  2  | 
  3  | const URL = "https://www.testmuai.com/selenium-playground/simple-form-demo/";
  4  | 
  ...
  14 | 
  15 |   await page.locator("#sum1").waitFor({
  16 |     state: "visible",
  17 |     timeout: 30000,
  18 |   });
  19 | });
  20 | 
  21 | test("To Test the input field", async ({ page }) => {
  22 |   const inputField = page.getByPlaceholder("Please enter your Message");
  23 |   const showButton = page.locator("#showInput");
  24 |   const output = page.locator("#message");
  25 | 
  26 |   await inputField.fill("prasanna");
  27 |   await showButton.click();
  28 | 
  29 |   await expect(output).toHaveText("prasanna", { timeout: 10000 });
  30 | 
  ...
  37 |   const button = page.locator("button", { hasText: "Get Sum" });
  38 |   const result = page.locator("#addmessage");
  39 | 
  40 |   await sum1.fill("25");
  41 |   await sum2.fill("10");
  42 | 
  43 |   await expect(sum1).toHaveValue("25");
  44 |   await expect(sum2).toHaveValue("10");
  45 | 
  46 |   await button.click();
  47 | 
> 48 |   await expect(result).toContainText("35", { timeout: 15000 });
     |                        ^ Error: expect(locator).toContainText(expected) failed
  49 | 
  50 |   console.log("Result:", await result.textContent());
  51 | });
```