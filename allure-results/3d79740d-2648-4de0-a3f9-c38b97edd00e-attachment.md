...
Locator:  locator('//div[@id = \'user-message\']//p[@id = \'addmessage\']')
Expected: visible
Received: hidden
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('//div[@id = \'user-message\']//p[@id = \'addmessage\']')
...
- main:
  ...
  - textbox "Please enter first value": "25"
  - paragraph: Enter second value
  - textbox "Please enter second value": "10"
  - button "Get Sum"
  - text: "Result:"
  - paragraph
- contentinfo:
...
```ts
  ...
  42 |   
  43 |   await sum1.pressSequentially('20', {delay:100})
  44 |   await sum1.fill(a);
  45 |   console.log(await sum1.inputValue());
  46 | 
  47 |   const sum2 = page.locator("#sum2");
  48 |   await sum2.scrollIntoViewIfNeeded();
  49 |   console.log(await sum2.boundingBox());
  50 |   await sum2.fill(b);
  51 |   console.log(await sum2.inputValue());
  52 | 
  53 |   await page.getByRole('button', { name: 'Get Sum' }).click();
  54 | 
  55 |   const display = page.locator("//div[@id = 'user-message']//p[@id = 'addmessage']");
  56 | 
  57 |   await page.waitForTimeout(1000);
  58 | 
  59 |     console.log("COUNT:", await display.count());
  60 |     console.log("TEXT:", await display.textContent());
  61 |     console.log("HTML:", await display.evaluate(el => el.outerHTML));
  62 | 
  63 | 
> 64 |   await expect(display).toBeVisible();
     |                         ^ Error: expect(locator).toBeVisible() failed
  65 | 
  66 |   await expect(display).toHaveText("35");
  67 | 
  68 | 
  69 | });
  70 | 
  71 | test.afterEach(async ({ page }) => {
